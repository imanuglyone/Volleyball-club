...
import { cancelProfileBooking, createProfileBookingV2, listProfileBookings } from './bookings';

describe('profile booking V2 isolation', () => {
  ...
  it('persists Mini App consent evidence through the V2 RPC', async () => {
    const rpc = vi.fn().mockResolvedValue({ data: [], error: null });
    await createProfileBookingV2({ rpc } as never, {
      trainingId: 'training',
      profileId: 'profile',
      name: 'Ира',
      phone: '+79001234567',
    ...
    expect(rpc).toHaveBeenCalledWith('create_profile_booking_v2', expect.objectContaining({
      p_consent_version: '2026-07-31-v1'
    }));
  });
});
