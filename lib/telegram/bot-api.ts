...
export function sendMessage(chatId: number, text: string, replyMarkup?: ReplyMarkup) {
  return callTelegram('sendMessage', { chat_id: chatId, text, ...(replyMarkup ? { reply_markup: replyMarkup } : {}) });
}
...
export function setAppMenuButton(chatId?: number) {
  ...
  return callTelegram('setChatMenuButton', {
    ...(chatId ? { chat_id: chatId } : {}), menu_button: { type: 'web_app', text: 'Открыть приложение', web_app: { url } }
  });
}
