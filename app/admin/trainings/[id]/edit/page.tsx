...
type EditTrainingPageProps = { params: Promise<{ id: string }> };
export const dynamic = 'force-dynamic';

export default async function EditTrainingPage({ params }: EditTrainingPageProps) {
  const { id } = await params;
  const supabase = createSupabaseAdminClient();
  const { data } = await supabase.from('trainings').select('id, date, start_time, end_time, price, capacity, is_active, location_name, address').eq('id', id).single();
  ...
  return <div className="admin-form-page">
    ...
    <AdminTrainingForm action={updateTraining.bind(null, id)} submitLabel="Сохранить изменения" defaultValues={{ date:data.date, start_time:data.start_time, end_time:data.end_time, price:data.price, capacity:data.capacity, location_name:data.location_name ?? '', address:data.address ?? '', is_active:data.is_active }} />
  </div>;
}
