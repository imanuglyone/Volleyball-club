...
export function TrainingCard({ training, featured = false }: { training: TrainingStats; featured?: boolean }) {
  ...
  return <article className={featured ? 'training-card featured' : 'training-card'}>
    <div className="card-top"><div><div className="eyebrow">{formatDate(training.date)}</div><h2>{formatTimeRange(training.start_time, training.end_time)}</h2></div><span className={`status ${status.key}`}>{status.label}</span></div>
    ...
    <Link className="button-primary" href={`/trainings/${training.id}`}>{training.remaining > 0 ? 'Открыть тренировку' : 'Посмотреть'}</Link>
  </article>;
}
