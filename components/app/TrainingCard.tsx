...
export function trainingStatus(training: TrainingSummary) {
...
type TrainingCardProps = { training: TrainingSummary; featured?: boolean; surface?: 'app' | 'public'; booked?: boolean };
...
export function TrainingCard({ training, featured = false, surface = 'app', booked = false }: TrainingCardProps) {
  ...
  const href = surface === 'app' ? `/app/trainings/${training.id}` : `/trainings/${training.id}`;
  return <article className={`training-card${featured ? ' training-card--featured' : ''} training-card--${surface}`}>
    ...
    <div className="training-card__footer"><span className="training-card__price">{training.price} ₽</span><Link className={buttonClassName({ variant: status.key === 'cancelled' ? 'ghost' : 'primary', size: 'sm' })} href={href}><span>{booked ? 'Открыть запись' : training.remaining > 0 && training.is_active ? 'Занять место' : 'Посмотреть'}</span><ArrowUpRight size={16}/></Link></div>
  </article>;
}
