...
export function TrainingFeed({ limit }: { limit?: number }) {
  ...
  return <>{!limit && <div className="date-strip"><button className={!date ? 'selected' : ''} onClick={() => setDate(null)}>Все</button>{dates.map((value) => <button key={value} className={date === value ? 'selected' : ''} onClick={() => setDate(value)}>{new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'short' }).format(new Date(`${value}T12:00:00`))}</button>)}</div>}<div className="training-list">{visible.map((item, index) => <TrainingCard key={item.id} training={item} featured={Boolean(limit && index === 0)}/>)}</div></>;
}
